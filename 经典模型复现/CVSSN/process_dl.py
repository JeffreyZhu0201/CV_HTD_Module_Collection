...
if __name__ == '__main__':
    ...
    # reshape as 
    gt_reshape = gt.reshape(-1)     # Flatten
    
    print(gt_reshape.shape) # (21025,)

    height, width, channels = data.shape
    class_count = max(np.unique(gt))    # 分类数

    flag_list = [0, 1]  # ratio or num  0为使用ratio划分，1为使用num划分
    ratio_list = [0.1, 0.01]  # [train_ratio,val_ratio]     # 训练、验证集划分
    ...
    num_list = [45, 4]  # [train_num,val_num]   # 训练、验证集划分
    ...
    data_padded = data_load_operate.data_pad_zero(data, patch_length)       # 加入padding防止变形
    height_patched, width_patched, channels = data_padded.shape             # 加入后的形状
