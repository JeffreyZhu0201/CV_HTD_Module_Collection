...
def attention_mechanism_fusion(tensor1, tensor2, p_type):
    se = AxialAttentionTransformer(dim = 65, depth = 2, reversible = True)
    tensor3 = se(tensor1)
    tensor4 = se(tensor2)
    tensor5 = (tensor3 + 1.0 * tensor4) * 0.6
    tensor6 = attention_fusion_channel(tensor3, tensor4, p_type)
    tensor7 = tensor6[1]
    tensor8 = tensor6[2]
    tensor9 = (tensor7 + tensor8) * 1.25
    return (tensor5 + tensor9) / 2
...
def channel_fusion(tensor1, tensor2, p_type):
    ...
    tensor_f = 0.8 * global_p_w1 * tensor1 + 0.8 * global_p_w2 * tensor2
...
def nuclear_pooling(tensor, kernel_size=None):
    ...
    for i in range(shape[1]):
        u, s, v = torch.linalg.svd(tensor[0, i, :, :] + EPSILON)
        s_sum = torch.sum(s)
...
if __name__ == '__main__':
    model = AxialAttentionTransformer(64,1, dim_index=1, reversible=True, axial_pos_emb_shape=(13,13))
    model.eval()
    print(model)
